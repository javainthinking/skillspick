...
export default function SkillCard({
  ...
  const sourceKind = s.sourceUrl?.includes("clawhub.ai")
    ? "clawhub"
    : s.repoUrl?.includes("github.com")
      ? "github"
      : null;
  const sourceLabel =
    sourceKind === "clawhub"
  ...
  return (
    ...
    >
      <div className="text-lg font-semibold leading-snug text-[color:var(--ui-fg)] transition group-hover:text-[color:var(--foreground)]">
        {s.name}
      </div>
      ...
      <div className="mt-3 line-clamp-2 text-base leading-relaxed text-[color:var(--ui-fg-muted)]">
        {s.description || ""}
      </div>

      <div className="mt-4 flex flex-wrap items-center gap-2 text-sm text-[color:var(--ui-fg-faint)]">
        <span className="rounded-full border border-[color:var(--ui-border)] bg-[color:var(--ui-bg)] px-2.5 py-1 transition group-hover:border-[color:var(--ui-border-strong)] group-hover:bg-[color:var(--ui-bg-hover)]">
          /{s.slug}
        </span>
        <span className="rounded-full border border-[color:var(--ui-border)] bg-[color:var(--ui-bg)] px-2.5 py-1 transition group-hover:border-[color:var(--ui-border-strong)] group-hover:bg-[color:var(--ui-bg-hover)]">
          ⭐ {s.stars ?? 0}
